...
class VersTipiDatoEndpointExceptionTest {
    ...
    void errorRequest() {
	when(serviceMock.listTipiDatoByVers(any(), any(), any(), any())).thenThrow(appGenericPersistenceException());
	given().when().queryParam("amb", "NO-amb").queryParam("vers", "NO-vers").get(URL_GET_VERS_TIPIDATO).then()
		.statusCode(500).body("$", hasKey(COD_ERR_INTERNAL));
    }
